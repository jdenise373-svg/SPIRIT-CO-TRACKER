...
export const AddEditContainerModal = ({
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      if (fillInputMethod === "weight") {
        finalCalcs = calculateDerivedValuesFromWeight(
          finalTare,
          parseFloat(formData.grossWeightLbs) || finalTare,
          finalProof
        );
      } else if (fillInputMethod === "wineGallons") {
        finalCalcs = calculateDerivedValuesFromWineGallons(
          parseFloat(wineGallonsInput) || 0,
          finalProof,
          finalTare
        );
      } else {
        // proofGallons
        finalCalcs = calculateDerivedValuesFromProofGallons(
          parseFloat(proofGallonsInput) || 0,
          finalProof,
          finalTare
        );
      }
      ...
      if (finalCalcs.netWeightLbs > 0.001) {
        ...
        if (!finalFillDate)
          finalFillDate = new Date().toISOString().split("T")[0];
      } else {
        newStatus = "empty";
      ...
      if (mode === "add") {
        ...
        // edit mode
        logNetChange =
          finalCalcs.netWeightLbs - (oldFillData.netWeightLbs || 0);
        ...
        } else if (container.status === "filled" && newStatus === "empty")
          logEntryType = TRANSACTION_TYPES.EDIT_EMPTY_FROM_FILLED;
        else if (newStatus === "filled")
          logEntryType = TRANSACTION_TYPES.EDIT_FILL_DATA_CORRECTION;
    ...
    const dataToSave = {
      ...
      type: formData.type,
      tareWeightLbs: finalTare,
      status: newStatus,
      currentFill: {
        productType:
          newStatus === "filled"
            ? finalProductType
            : oldFillData.productType && newStatus === "empty"
            ? oldFillData.productType
            : null,
    ...
    try {
      ...
      const logData = {
        ...
        containerName: dataToSave.name,
        productType: dataToSave.currentFill.productType,
        proof: dataToSave.currentFill.proof,
        netWeightLbsChange: logNetChange,
        proofGallonsChange: logPgChange,
        notes: `${mode} via ${fillInputMethod} method. ${
          isAddingEmpty
            ? "New empty."
            : newStatus === "empty" && logEntryType !== "CREATE_EMPTY_CONTAINER"
            ? "Container emptied."
            : ""
        }`,
      };
