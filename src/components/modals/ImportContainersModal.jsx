...
export const ImportContainersModal = ({
  ...
  const parseCSV = (csvFile) => {
    ...
    reader.onload = (e) => {
      ...
      if (lines.length < 2) {
        setLocalError("CSV must have a header and at least one data row.");
        return;
      }
      ...
      if (!requiredHeaders.every((h) => headers.includes(h))) {
        setLocalError(
          `CSV header must contain: ${requiredHeaders.join(", ")}.`
        );
        return;
      }
      ...
      const data = lines
        ...
        .map((line, index) => {
          ...
          const rowData = {};
          headers.forEach((h, i) => {
            rowData[h.replace(/\s+/g, "_")] = values[i] || "";
          });
          ...
          else if (!Object.keys(CONTAINER_CAPACITIES_GALLONS).includes(type))
          ...
          if (status === "filled") {
            ...
            if (!productType || !products.some((p) => p.name === productType))
              errors.push("Invalid Product Type.");
            if (!fillDate || isNaN(new Date(fillDate)))
              errors.push("Invalid Fill Date.");
            if (isNaN(proof) || proof < 0 || proof > 200)
              errors.push("Invalid Proof.");

            let calcs;
            if (!isNaN(gross) && gross > tare)
              calcs = calculateDerivedValuesFromWeight(tare, gross, proof);
            else if (!isNaN(net) && net > 0)
              calcs = calculateDerivedValuesFromWeight(tare, tare + net, proof);
            else if (!isNaN(wg) && wg > 0)
              calcs = calculateDerivedValuesFromWineGallons(wg, proof, tare);
            else if (!isNaN(pg) && pg > 0)
              calcs = calculateDerivedValuesFromProofGallons(pg, proof, tare);
            else {
              errors.push("No valid fill data (gross, net, wg, or pg).");
              calcs = calculateDerivedValuesFromWeight(tare, tare, 0);
            }

            containerData.currentFill = {
              productType,
              fillDate,
              proof,
              ...calcs,
            ...
            containerData.currentFill = {
              ...calculateDerivedValuesFromWeight(tare, tare, 0),
              emptiedDate:
                rowData.empty_date || new Date().toISOString().split("T")[0],
            };
          }
  ...
  const handleImport = async () => {
    ...
    if (validData.length === 0) {
      setLocalError("No valid containers to import.");
      return;
    }
    ...
    logTransaction(db, userId, appId, {
      type: "CREATE_BULK_CONTAINERS",
      notes: `Imported ${validData.length} containers via CSV.`,
    });
    ...
    try {
      ...
      setLocalError("Import failed: " + err.message);
      setErrorApp("Import failed.");
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center p-4 z-50">
      <div className="bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col">
        ...
        {error && (
          <div className="bg-red-600 p-3 rounded mb-4 text-sm">{error}</div>
        )}
        <div className="space-y-4">
          <p className="text-sm text-gray-400">
            CSV must include headers:{" "}
            <strong>container name, type, tare</strong>. Optional headers for
            filled containers:{" "}
            <strong>
              status, product type, proof, fill date, gross, net, wine gallons,
              proof gallons, empty date
            </strong>
            .
          </p>
          <input
                  ...
                  <tbody>
                    {parsedData.map((row, i) => (
                      ...
                      >
                        ...
                        <td className="p-2">
                          {row.currentFill?.productType || "N/A"}
                        </td>
                        <td className="p-2">
                          {row.currentFill?.proofGallons?.toFixed(2) || "N/A"}
                        </td>
                        <td
        ...
        <div className="flex justify-end space-x-3 pt-4 mt-auto">
          ...
          <button
            ...
            className="bg-blue-600 py-2 px-4 rounded disabled:bg-gray-500 disabled:cursor-not-allowed"
          >
            Import {validRows} Containers
          </button>
        </div>
      </div>
    </div>
