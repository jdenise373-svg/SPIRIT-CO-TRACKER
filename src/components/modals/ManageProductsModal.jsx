...
export const ManageProductsModal = ({
  ...
  const handleGenerateDescription = async () => {
    if (!newProductName.trim()) {
      setFormError(
        "Please enter a product name first to generate a description."
      );
      return;
    }
    setFormError("");
    ...
    try {
      ...
      setFormError(
        "Failed to generate description: " +
          err.message +
          ". Ensure API key is correctly configured if testing locally."
      );
    } finally {
  ...
  const handleSaveProduct = async () => {
    ...
    if (
      ...
      setFormError(
        `Another product with the name "${newProductName.trim()}" already exists.`
      );
      return;
    }
        ...
        <div className="space-y-4 mb-6">
          ...
          <div>
            ...
            <button
              ...
              disabled={isGeneratingDesc || !newProductName.trim()}
              className="mt-2 text-sm bg-purple-600 hover:bg-purple-700 text-white font-semibold py-1 px-3 rounded-md disabled:opacity-50 disabled:cursor-not-allowed"
            >
        ...
        <div className="flex-grow overflow-y-auto pr-2">
          ...
          <ul className="space-y-3">
            {currentProducts.map((product) => (
              <li
                key={product.id}
                className="flex items-start justify-between gap-4 bg-gray-700 p-3 rounded-md"
              >
