...
import {
  CONTAINER_CAPACITIES_GALLONS,
  DENSITY_WATER_LBS_PER_GALLON,
  TRANSACTION_TYPES,
} from "../../constants";
...
export const ProofDownModal = ({
  ...
  const calculations = useMemo(() => {
    ...
    if (isNaN(newProof) || newProof <= 0 || newProof >= initialProof) {
      return {
        waterToAddGallons: 0,
        waterToAddLbs: 0,
        finalWineGallons: initialWineGallons,
        finalGrossWeight: 0,
        finalSpiritDensity: 0,
        isValid: false,
      };
    }
  ...
  const handleProofDown = async () => {
    ...
    if (!calculations.isValid) {
      setFormError(
        `Invalid target proof. Must be > 0 and less than the current proof of ${initialProof}.`
      );
      return;
    }
    ...
    if (capacity > 0 && calculations.finalWineGallons > capacity) {
      setFormError(
        `Resulting volume (${calculations.finalWineGallons.toFixed(
          2
        )} gal) exceeds container capacity (${capacity} gal).`
      );
      return;
    }
    ...
    const updatedData = {
      ...
      "currentFill.netWeightLbs":
        (currentFill.netWeightLbs || 0) + calculations.waterToAddLbs,
    ...
    const logData = {
      ...
      notes: `Proofed down from ${initialProof} to ${newProof}. Added ${calculations.waterToAddGallons.toFixed(
        2
      )} gal of water.`,
    };
      ...
      <div className="bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-md">
        ...
        <div className="space-y-4">
          <div className="text-sm text-gray-400">
            ...
            <p>
              <strong>Current State:</strong> {initialWineGallons.toFixed(2)} WG
              @ {initialProof} Proof ({initialProofGallons.toFixed(2)} PG)
            </p>
          </div>
          <div>
            <label
            ...
            <input
              ...
              step="0.1"
              min="0"
              max={initialProof > 0 ? initialProof - 0.1 : 0}
            />
          </div>
            ...
            <div className="bg-gray-700 p-3 rounded border border-gray-600 text-sm">
              ...
              <p>
                ...
                <span className="text-cyan-300">
                  {calculations.waterToAddGallons.toFixed(2)} gal
                </span>{" "}
                ({calculations.waterToAddLbs.toFixed(2)} lbs)
              </p>
              <hr className="my-2 border-gray-600" />
              <p>
                <strong>Final WG:</strong>{" "}
                {calculations.finalWineGallons.toFixed(2)} gal
              </p>
              <p>
                <strong>Final Gross Wt:</strong>{" "}
                {calculations.finalGrossWeight.toFixed(2)} lbs
              </p>
              <p className="font-bold">
                <strong>Final Proof Gallons:</strong>{" "}
                {initialProofGallons.toFixed(2)} PG (Unchanged)
              </p>
            </div>
          )}
          <div className="flex justify-end space-x-3 pt-3">
            ...
            <button
              ...
              className="bg-cyan-600 py-2 px-4 rounded disabled:bg-cyan-800 disabled:cursor-not-allowed"
            >
              Confirm Proof Down
            </button>
          </div>
