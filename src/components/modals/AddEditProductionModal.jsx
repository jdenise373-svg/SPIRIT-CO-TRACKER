...
export const AddEditProductionModal = ({
  ...
  const filledStorageTanks = useMemo(() => {
    return inventory
      .filter((c) => c.status === "filled" && c.type !== "still")
      .map((c) => ({
        id: c.id,
        name: c.name,
        type: c.type,
        displayName: `${c.name} (${c.type.replace(
          /_/g,
          " "
        )}) - ${c.currentFill?.proofGallons?.toFixed(2)} PG`,
  ...
  useEffect(() => {
    if (type === "fermentation") {
      ...
      if (batch) {
        if (batch.chargeInputMethod)
          setChargeInputMethod(batch.chargeInputMethod);
        ...
        if (batch.selectedContainerId)
          setSelectedContainerId(batch.selectedContainerId);
        if (batch.selectedStorageTankId)
          setSelectedStorageTankId(batch.selectedStorageTankId);
        if (batch.storageTankPullAmount)
          setStorageTankPullAmount(batch.storageTankPullAmount);
        if (batch.storageTankPullMethod)
          setStorageTankPullMethod(batch.storageTankPullMethod);
      }
    }
  }, [batch, type]);
  ...
  useEffect(() => {
    ...
    if (chargeValue > 0 && chargeProof > 0) {
      ...
      if (chargeInputMethod === "weight") {
        calculated = calculateDerivedValuesFromWeight(
          0,
          chargeValue,
          chargeProof,
          temp
        ...
        calculated = calculateDerivedValuesFromWineGallons(
          chargeValue,
          chargeProof,
          0,
          temp
        ...
        // proofGallons
        calculated = calculateDerivedValuesFromProofGallons(
          chargeValue,
          chargeProof,
          0,
          temp
  ...
  useEffect(() => {
    if (selectedStorageTankId && storageTankPullAmount) {
      const selectedTank = filledStorageTanks.find(
        (t) => t.id === selectedStorageTankId
      );
      if (selectedTank && selectedTank.currentFill) {
        ...
        if (pullAmount > 0 && tankProof > 0) {
          ...
          setFormData((prev) => ({
            ...prev,
            chargeProof: tankProof.toString(),
          }));
        }
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      setFormError(
        "Please select a container to store the distillation yield."
      );
      return;
    }
    ...
    if (
      ...
      if (
        ...
        if (pullAmount > 0 && tankProof > 0) {
          let pullCalculated;
          if (storageTankPullMethod === "weight") {
            pullCalculated = calculateDerivedValuesFromWeight(
              0,
              pullAmount,
              tankProof
            );
          } else if (storageTankPullMethod === "wineGallons") {
            pullCalculated = calculateDerivedValuesFromWineGallons(
              pullAmount,
              tankProof,
              0
            );
            ...
            // proofGallons
            pullCalculated = calculateDerivedValuesFromProofGallons(
              pullAmount,
              tankProof,
              0
            );
          ...
          if (
            pullCalculated.proofGallons >
            sourceTank.currentFill.proofGallons + 0.001
          ) {
            setFormError(
              `Cannot pull ${pullCalculated.proofGallons.toFixed(
                2
              )} PG from tank that only has ${sourceTank.currentFill.proofGallons.toFixed(
                2
              )} PG.`
            );
            return;
    ...
    try {
      ...
      if (type === "distillation" && yieldCalculated.proofGallons > 0) {
        ...
        if (!isEdit && selectedContainerId) {
          const selectedContainer = inventory.find(
          ...
          if (selectedContainer && selectedContainer.status === "empty") {
            ...
            const containerLogData = {
              ...
              notes: `Filled with ${yieldCalculated.proofGallons.toFixed(
                2
              )} PG from distillation batch ${dataToSave.name}.`,
            };
            batchRef.set(doc(logCollRef), {
              ...containerLogData,
              timestamp: serverTimestamp(),
      ...
      if (
        ...
        const sourceTank = inventory.find(
          (c) => c.id === selectedStorageTankId
        );
        if (
          sourceTank &&
          sourceTank.status === "filled" &&
          ...
          if (pullAmount > 0 && tankProof > 0) {
            ...
            const remainingNetWeight = Math.max(
              0,
              sourceTank.currentFill.netWeightLbs - pullCalculated.netWeightLbs
            );
            const remainingWineGallons = Math.max(
              0,
              sourceTank.currentFill.wineGallons - pullCalculated.wineGallons
            );
            const remainingProofGallons = Math.max(
              0,
              sourceTank.currentFill.proofGallons - pullCalculated.proofGallons
            );
            const remainingGrossWeight =
              sourceTank.tareWeightLbs + remainingNetWeight;

            // Update source tank
            const sourceTankRef = doc(
              db,
              `artifacts/${appId}/users/${userId}/spiritInventory`,
              selectedStorageTankId
            );
            ...
            const pullLogData = {
              ...
              notes: `Pulled ${pullCalculated.proofGallons.toFixed(
                2
              )} PG for distillation batch ${dataToSave.name}.`,
            };
            batchRef.set(doc(logCollRef), {
              ...pullLogData,
              timestamp: serverTimestamp(),
              ...
              {formData.sourceBatchId === "storage_tank" && (
                <div className="border border-gray-600 rounded p-4 bg-gray-750">
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-3">
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setSelectedStorageTankId(e.target.value)
                        }
                        className="w-full bg-gray-700 p-2 rounded"
                      >
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setStorageTankPullMethod(e.target.value)
                        }
                        className="w-full bg-gray-700 p-2 rounded"
                      >
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-300 mb-1">
                        {storageTankPullMethod === "weight"
                          ? "Weight (lbs)"
                          : storageTankPullMethod === "wineGallons"
                          ? "Wine Gallons"
                          : "Proof Gallons"}
                      </label>
                      <input
                        ...
                        onChange={(e) =>
                          setStorageTankPullAmount(e.target.value)
                        }
                        placeholder={`Enter ${
                          storageTankPullMethod === "weight"
                            ? "weight"
                    ...
                    <div className="flex flex-col justify-end">
                      <div className="text-sm text-gray-400">
                        <div>
                          Net Weight: {chargeCalculated.netWeightLbs.toFixed(2)}{" "}
                          lbs
                        </div>
                        <div>
                          Wine Gallons:{" "}
                          {chargeCalculated.wineGallons.toFixed(2)} gal
                        </div>
                        <div>
                          Proof Gallons:{" "}
                          {chargeCalculated.proofGallons.toFixed(2)} PG
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-3">
                  ...
                  <div className="flex flex-col justify-end">
                    <div className="text-sm text-gray-400">
                      ...
                      <div>
                        Wine Gallons: {chargeCalculated.wineGallons.toFixed(2)}{" "}
                        gal
                      </div>
                      <div>
                        Proof Gallons:{" "}
              ...
              <div className="border border-gray-600 rounded p-4 bg-gray-750">
                ...
                <div className="grid grid-cols-2 gap-4 mb-3">
                  ...
                  <div className="flex flex-col justify-end">
                    <div className="text-sm text-gray-400">
                      <div>
                        Net Weight: {yieldCalculated.netWeightLbs.toFixed(2)}{" "}
                        lbs
                      </div>
                      <div>
                        Wine Gallons: {yieldCalculated.wineGallons.toFixed(2)}{" "}
                        gal
                      </div>
                      <div>
                        Proof Gallons: {yieldCalculated.proofGallons.toFixed(2)}{" "}
                        PG
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-4">
                    ...
                    {emptyContainers.length === 0 && (
                      <p className="text-yellow-400 text-sm mt-1">
                        No empty containers available. Please create an empty
                        container first.
                      </p>
                    )}
                  </div>
